...
function parseExpressions(expressions) {
  ...
  parameters.forEach((parameter) => {
    parsedExpressions[parameterToVarMap[parameter]] = expressions[parameter]
      .map((expression) => {
        ...
        return vars.reduce((accumulator, varToReplaceWith) => {
          ...
          while (expressionProcessing.indexOf(parameterToReplace) !== -1) {
            expressionProcessing = expressionProcessing
              .replace(parameterToReplace, varToReplaceWith);
          }
